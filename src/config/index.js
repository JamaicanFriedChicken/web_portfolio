module.exports = {
  siteTitle: 'Michael Lue | Software Engineer',
  siteDescription:
    'Michael is an aspiring software developer from Kingston, Jamaica specializing in building, quality websites and applications as well as reverse engineering anything I can get my hands on.',
  siteKeywords:
    'Michael Lue, Michael, Lue, jamaicanfriedchicken, software engineer, front-end engineer, web developer, javascript, zhejiang university',
  siteUrl: 'https://michaellue.netlify.com/',
  siteLanguage: 'en_US',
  googleAnalyticsID: 'UA-157839180-1',
  googleVerification: 'NXopfU6nCxJb9eGPKN56Dmy-y6OXMDA_VdZ99BgHgMY',
  name: 'Michael Lue',
  location: 'Kingston, JA',
  email: 'mikey_lue777@hotmail.com',
  github: 'https://github.com/JamaicanFriedChicken',
  socialMedia: [
    {
      name: 'GitHub',
      url: 'https://github.com/JamaicanFriedChicken',
    },
    {
      name: 'Linkedin',
      url: 'https://www.linkedin.com/in/mlue/',
    },
  ],

  navLinks: [
    {
      name: 'About',
      url: '/#about',
    },
    {
      name: 'Experience',
      url: '/#jobs',
    },
    {
      name: 'Work',
      url: '/#projects',
    },
    {
      name: 'Contact',
      url: '/#contact',
    },
  ],

  navHeight: 100,
  greenColor: '#64ffda',
  navyColor: '#0a192f',
  darkNavyColor: '#020c1b',

  srConfig: (delay = 200) => ({
    origin: 'bottom',
    distance: '20px',
    duration: 500,
    delay,
    rotate: { x: 0, y: 0, z: 0 },
    opacity: 0,
    scale: 1,
    easing: 'cubic-bezier(0.645, 0.045, 0.355, 1)',
    mobile: true,
    reset: false,
    useDelay: 'always',
    viewFactor: 0.25,
    viewOffset: { top: 0, right: 0, bottom: 0, left: 0 },
  }),
};
